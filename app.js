...
async function registerUser(user, pass) {
  ...
  if (existing) {
    console.warn('⚠️ Usuario ja existe:', user);
  ...
  console.log('📋 Total de usuarios agora:', db.users.length);
...
document.getElementById('register-cancel').addEventListener('click', closeRegister);
document.getElementById('register-overlay').addEventListener('click', function(e) {
...
document.getElementById('register-form').addEventListener('submit', async function(e) {
  ...
  // Validations
  if (!user) { errEl.textContent = 'digite um usuario'; return; }
  if (user.length < 2) { errEl.textContent = 'usuario deve ter minimo 2 caracteres'; return; }
  if (user.length > 30) { errEl.textContent = 'usuario nao pode ter mais de 30 caracteres'; return; }
  if (!/^[a-zA-Z0-9_-]+$/.test(user)) { errEl.textContent = 'usuario so pode conter letras, numeros, _ e -'; return; }
  ...
  try {
    console.log('🔄 Registrando novo usuário:', user);
    const result = await registerUser(user, pass);
    
    if (result.success) {
      console.log('✓ Registro bem-sucedido para:', user);
      
      // Verify user was created
      const users = getAllUsers();
      const created = users.find(u => u.username === user);
      if (created) {
        console.log('✓ Verificado: novo usuario existe no banco');
      } else {
        console.warn('⚠️ Aviso: usuario nao encontrado apos registro');
      }
      ...
      setTimeout(async () => {
        console.log('🔄 Tentando login automático para:', user);
        ...
        if (loginResult.valid) {
          ...
          errEl.textContent = 'registro pronto! faça login agora.';
          submitBtn.disabled = false;
          submitBtn.textContent = originalText;
        }
      }, 500);
    } else {
      console.warn('Registro falhou:', result.error);
    ...
    errEl.textContent = 'erro ao registrar. tente novamente';
    submitBtn.disabled = false;
    submitBtn.textContent = originalText;
  }
});
